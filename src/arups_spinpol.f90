...
!                                              (qplusGy - 0)    *(qplusGy - 0))
                             deltak   = dsqrt((qplusGx - kxarr)*(qplusGx - kxarr) +&
                              ...
                              photoint(ikx,iek) = photoint(ikx,iek) + fermifac*l1*l2*cgcabs
                              if (.not. pureplanewave) then
                                photoint2(ikx,iek)= photoint2(ikx,iek)+ fermifac*l1*l2*cgcabs2
                              end if                                     
                            end if  ! k range               
...
!                                               * exp(-(qplusGy - kyarr)*(qplusGy - kyarr)/(2*kybroad*kybroad) )    
                            ...
                            photoint(ikx,iky) = photoint(ikx,iky) + fermifac*l1*l2*cgcabs
                            if (.not. pureplanewave) then
                              photoint2(ikx,iky)= photoint2(ikx,iky)+ fermifac*l1*l2*cgcabs2
                            end if                                        
                          end if                 
...
!                                            (qplusGy - 0)    *(qplusGy - 0))
                            ...
                            l2 = (1/(kxbroad*sqrt(two_pi)))*exp(-deltak*deltak/(2*kxbroad*kxbroad))
                            photoint(ikx,ieph) = photoint(ikx,ieph) + fermifac*l1*l2*cgcabs
                            if (.not. pureplanewave) then
                              photoint2(ikx,ieph)= photoint2(ikx,ieph)+ fermifac*l1*l2*cgcabs2
                            end if                                       
                          end if                       
                        end do  ! end loop over kx values  
                      end if  ! ikxend .ge. ikxstart
